...
class logic_mgr_simulator(object):
	...
	def retrieve_ought_to_buy_items(self, ii_obj, available_price, d_range, print_detail=False):
		...
		del p_mgr_ii
		gc.collect()
		print("gc checkpoint ---%d"%1)

		predicted_df["weight"] = 1 - predicted_df["atr_40"] / predicted_df["low_price"]
		predicted_df["weight"] = predicted_df["weight"].astype('float32')  # Change data type to reduce memory consumption ---
		predicted_df["required_budget"] = predicted_df["high_price"] / predicted_df["weight"]
		predicted_df["required_budget"] = predicted_df["required_budget"].astype('float32')  # Change data type to reduce memory consumption ---
		id_lst = predicted_df["stock_id"].drop_duplicates().values
		print(id_lst)
		sql_str = "WHERE stock_id in ("
		for cur_id in id_lst:
			sql_str = sql_str + "'%s',"%str(cur_id)
		sql_str = sql_str[0:len(sql_str)-1] + ")"
		stock_details = self.crud.read_tbl_by_df(sql_str).loc[:, ["stock_id", "stock_unit"]]
		stock_details["stock_id"] = stock_details["stock_id"].astype('int16')
		...
		predicted_df["min_predict_close_price"] = predicted_df["min_predict_close_price"].astype('int16')  # Change data type to reduce memory consumption ---
		del min_predict_prices
		gc.collect()
		print("gc checkpoint ---%d"%3)
		...
		predicted_df["min_pre_predict_close_price"] = predicted_df["min_pre_predict_close_price"].astype('int16')  # Change data type to reduce memory consumption ---
		del min_pre_predict_prices
		gc.collect()
		print("gc checkpoint ---%d"%4)
		# Insert [move_avrg_40_prev]
		predicted_df["move_avrg_40"] = predicted_df["move_avrg_40"].astype('float32')  # Change data type to reduce memory consumption ---
		prev_lst = predicted_df.loc[:, ["stock_id", "date", "move_avrg_40"]]
		prev_lst = prev_lst.drop_duplicates()
		for n in range(10):
		...
		prev_lst["move_avrg_40_prev"] = prev_lst["move_avrg_40_prev"].astype('float32')  # Change data type to reduce memory consumption ---
		for n in range(1,10):
	...
	def apply_sell_rule(self, held_stock_lst, available_price, target_date, predicted_df, update_fl=False):

		print("apply sell rule.....")
		...
		predicted_df = predicted_df.loc[predicted_df["date"]<target_date, ["stock_id", "date", "predict_low_price"]]
		predicted_df = predicted_df.groupby(["stock_id","date"],as_index=False).min().sort_values(by=["date"], ascending=True)
		...
		for cur_hid in range(len(held_stock_lst)):
			...
			print("%s is being checkied......."%str(cur_id))
			...
			preb_items = predicted_df.loc[predicted_df["stock_id"]==int(cur_id), :]
			preb_items = preb_items.iloc[len(preb_items)-1]
			...
			# 当日の終値が安値の予測値の最小値より小さい
			conditions.append(preb_items["predict_low_price"] > latest_item["close_price"])
			sell_point = sell_point+1 * (preb_items["predict_low_price"] - latest_item["close_price"])
			##############
			print(conditions)
