...
from crud_for_news import CRUD_for_KLUG_FX
...
crud = CRUD_for_KLUG_FX()

driver = webdriver.PhantomJS() 

def main():
	# The oldest link is http://klug-fx.jp/fxnews/daily.php?ymd=2008-02-01
	base_url = "http://klug-fx.jp/fxnews/daily.php?ymd=%s"
...
def get_news(base_url, d_str):
	...
	a_lst = driver.find_elements_by_css_selector("ul.phase1_article_list > li > a")
	num = 0
	for a in a_lst:
	...
	for url in url_lst:
		if(is_downloaded(url)):
			...
			try:
				...
				driver.get(url)
				title = driver.find_elements_by_css_selector("div#phase1_contents > div.phase1_box")[1].find_element_by_css_selector("h1").text
				contents = driver.find_element_by_css_selector("p.phase1_texts").text
				uploaded_date = driver.find_element_by_css_selector("dl.phase1_publish_time > dd").text
				cur_json = {
